...
public:
  ...
  /// Returns the first renderer attached to the pipeline.
  /// For an exhaustive list use \sa GetRenderers
  ///
  /// \sa OnRendererAdded
  ...
  bool UpdateObserver(vtkObject* prevObj, vtkObject* obj, int n_events, unsigned long events[]) const VTK_SIZEHINT(events, n_events);
  /// @}
